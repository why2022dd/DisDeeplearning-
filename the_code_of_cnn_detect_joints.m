...
X=xlsread('training_data.xlsx');%The dataset is the final training data carefully selected by the operator.
...
%
XX=xlsread('test_data.xlsx');%The dataset is the final test data carefully selected by the operator.
 Xtest1=XX(:,1:6);
 for ii=1:1:252
    for jj=1:1:6    
...
clc
% close all '
load ('All point cloud data.mat');%Apply to all point clouds
Xtest1=pcData(:,1:6);
for ii=1:1:2161140
    for jj=1:1:6
        Xtest(1,jj,1,ii)=Xtest1(ii,jj);
